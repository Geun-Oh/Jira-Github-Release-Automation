...
let domain, project, version, token, isCreateNextVersion;
(async () => {
  try {
    ...
    isCreateNextVersion = core.getInput("create-next-version");

    const versionName = getVersionName(version);
    if (versionName == null) {
      core.setFailed(
        `version is not correct: [${version}] must be '1.0.0'/'v1.0.0'/'test 1.0.0' pattern`
      );
      return;
    }
...
async function getVersions(appName) {
  const uri = `https://${domain}.atlassian.net/rest/api/3/project/${project}/version?query=${encodeURI(
    appName
  )}&orderBy=name&status=unreleased`;
  const options = {
    method: "GET",
    uri: uri,
    headers: {
